...
from models.cdlm import CDLMClassifier
from models.cdlm_bert import CDLMBertClassifier
...
#
# misclassified_logger = logging.getLogger('misclassified_logger')
# misclassified_logger.setLevel(logging.INFO)
# file_handler = logging.FileHandler('misclassified_samples.log')
# formatter = logging.Formatter('%(asctime)s - %(message)s')
# file_handler.setFormatter(formatter)
# misclassified_logger.addHandler(file_handler)
...
class Instructor:
    ...
    def __init__(self, opt):
        ...
        if 'bert' in opt.model_name:
            ...
            tokenizer = Tokenizer4BertGCN(opt.max_length, opt.local_pretrained_path)
            bert = BertModel.from_pretrained(opt.local_pretrained_path)
            ...
            vocab_help = (post_vocab, pos_vocab, dep_vocab, pol_vocab)  # vocab_help是一个元组图tuple
            ...
            for i_batch, sample_batched in enumerate(self.train_dataloader):
                ...
                if global_step % self.opt.log_step == 0:
                    ...
                    logger.info('loss: {:.4f}, acc: {:.4f}, test_acc: {:.4f}, f1: {:.4f}'.format(loss.item(), train_acc,
                                                                                                 test_acc, f1))
            epoch_end = time.time()
            logger.info("Epoch {} finished in {:.2f} seconds".format(epoch, epoch_end - epoch_start))

        total_end = time.time()
        # <<< 新增：打印总用时
        logger.info(
            "Total training time for {} epochs: {:.2f} seconds".format(self.opt.num_epoch, total_end - total_start))
    ...
    def _measure_latency(self, n_iter=100, repeat=5):
        """ 测单样本推理延迟，多次重复取平均 """
        self.model.eval()
        ...
        # warm-up
        with torch.no_grad():
            for i, sample_batched in enumerate(single_sample_loader):
                if i >= 5:
                    break
                inputs = [sample_batched[col].to(self.opt.device) for col in self.opt.inputs_cols]
                _ = self.model(inputs)
        ...
        for r in range(repeat):
            ...
            avg_latency_ms = (total_time / n_iter) * 1000
            repeat_latencies.append(avg_latency_ms)
        final_latency = sum(repeat_latencies) / len(repeat_latencies)
        logger.info(
            f"Average single-sample latency over {n_iter} samples repeated {repeat} times: {final_latency:.4f} ms")
    ...
    def run(self):
        ...
        self._test()
        latency = self._measure_latency(n_iter=100, repeat=5)
        logger.info("Average latency: {:.4f} s per sample".format(latency))


def main():
    model_classes = {
        'cdlm': CDLMClassifier,
        'cdlmbert': CDLMBertClassifier,
    ...
    input_colses = {
        ...
        'cdlm': ['text', 'aspect', 'pos', 'head', 'deprel', 'post', 'mask', 'length', 'short_mask'],
        'cdlmbert': ['text_bert_indices', 'bert_segments_ids', 'attention_mask', 'asp_start', 'asp_end', 'src_mask',
    ...
    parser.add_argument('--model_name', default='cdlm', type=str, help=', '.join(model_classes.keys()))
    parser.add_argument('--dataset', default='laptop', type=str, help=', '.join(dataset_files.keys()))
    parser.add_argument('--optimizer', default='adam', type=str, help=', '.join(optimizers.keys()))
    parser.add_argument('--initializer', default='xavier_normal_', type=str, help=', '.join(initializers.keys()))
    ...
    parser.add_argument('--local_pretrained_path', type=str, default='./bert-base-uncased')

    parser.add_argument('--in_dim', type=int, default=100, help='Input dimension size')
    parser.add_argument('--num_feature_maps', type=int, default=256, help='Number of dpcnn feature maps')
    parser.add_argument('--gcn', type=str, default='gat', help='gcn or gat')
